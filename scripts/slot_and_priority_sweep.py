...
def run_sim(tickers_data, calendar, entry_style, scope_lo_hi,
    ...
    for day_idx, day in enumerate(calendar):
        ...
        if priority == "gap_desc":
            fill_candidates.sort(key=lambda x: (-abs(x[5]), x[0]))  # biggest |gap%| first, alpha tiebreak
        else:
...
def main():
    ...
    # wick/all only -- isolate whether MORE SLOTS or a BETTER TIE-BREAK (or
    # both) fixes the crowding-out found in the 18-backtest run.
    slot_counts = [20, 50, 100, 200]
    priorities = ["alpha", "gap_desc"]
    summary_rows = []
    for slots in slot_counts:
        for pri in priorities:
            label = f"slots={slots} priority={pri}"
            print(f"\nrunning wick/all, {label} ...")
            r = run_sim(tickers_data, calendar, "wick", "all",
                       max_slots=slots, priority=pri)
            fname = f"wick_all_slots{slots}_{pri}"
            r["trades_df"].to_csv(LOGS_DIR / f"{fname}.csv", index=False)
            r["equity_curve"].to_csv(LOGS_DIR / f"{fname}_equity.csv")
            bucket_share = (r["trades_df"]["bucket"].value_counts(normalize=True) * 100).round(1).to_dict() if len(r["trades_df"]) else {}
            summary_rows.append({
                "max_slots": slots, "priority": pri,
                "n_trades": r["n_trades"],
                ...
                "ret_per_maxdd": round(r["total_return_pct"] / abs(r["max_drawdown_pct"]), 3) if r["max_drawdown_pct"] else None,
                "pct_1-2%_bucket": bucket_share.get("1-2%"),
                "pct_10%+_bucket": bucket_share.get("10%+"),
            })
            print(f"  n={r['n_trades']}  win%={r['win_rate_pct']}  "
                  ...
                  f"bucket mix: {bucket_share}")

    summary = pd.DataFrame(summary_rows)
    summary.to_csv((HERE.parent / "results" / "slot_priority_sweep_summary.csv"), index=False)
    print("\n" + "=" * 140)
    print("SLOT COUNT x PRIORITY SWEEP -- wick/all, 2yr")
    print("=" * 140)
